...
function ConvertCADataUnit(sDataValue)
{
    var t= (parseFloat((sDataValue).replace(/,/g, '').replace('%', '').replace(/NA/g, null)) * gUnit_CAData).toFixed(2).toString();      
    if(t== 'NaN')
    {
        return sDataValue;
    }
...
function ConvertCADataUnitChart(sDataValue)
{
    var t= (parseFloat((sDataValue).replace(/,/g, '').replace('%', '').replace(/NA/g, null)) * gUnit_CAData).toFixed(2).toString();      
    if(t== 'NaN')
    {
...
//dsfsdfsdfs
    ...
    try {
        ...
        $('#tableContainerCA').html('');

        $.each(gValuationDataCA, function (index, ldata) {

            ValuationData = ldata.PANEL_DATA;
            //            alert("ValuationDataRatio :" + JSON.stringify(ValuationData));
            if (ValuationData.LEVELDATA.length > 0) {

                table = $("<table class=\"tree table table-striped fixedHeader_collapsetree\" style=\"width: 98% !important; margin-bottom:20px !important;\"></table>").appendTo($('#tableContainerCA'));
            ...
            for (var i = 0; i < ValuationData.LEVELDATA.length; i++) {
                ...
                $.each(lLevelData.DATA_VAL, function (index, slist) {
                   if(index >=gStartIndex && index <= gEndIndex)
                   {
                    ...
                    MyTempArray.push(ConvertCADataUnitChart(slist));
                  }
                });
                ...
                $.each(lLevelData.DATA_VAL, function (index, sDataValue) {
                       if(index >=gStartIndex && index <= gEndIndex)
                    {
                    var td = $("<td></td>").html('<div class=\"treegrid-Cell\">' + (sDataValue == "PREMIUM" ? "<div class='blur' title='Premium'> &nbsp;<div/>" : ConvertCADataUnit(sDataValue)) + '</div>');
                    tr.append(td);
                }
                });
                ...
                if (lLevelData.CHILD_DATA != null) {
                    ...
                    for (var j = 0; j < lChildData.length; j++) {
                        ...
                        $.each(ObjVal.DATA_VAL, function (index, sYearData) {
                               if(index >=gStartIndex && index <= gEndIndex)
                    {
                            var td = $("<td></td>").html('<div class=\"treegrid-Cell\">' + (sYearData == "PREMIUM" ? "<div class='blur' title='Premium'> &nbsp;<div/>" : ConvertCADataUnit(sYearData)) + '</div>');
                            trC.append(td);
                        }
                        });
