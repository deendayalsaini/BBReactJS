...
//alert("inside multiple_emails js loaded");
    ...
    $.fn.multiple_emails = function (options) {
            //alert("email data");
        // Default options
      //  alert("inside multiple_emails js loaded 2");
        var defaults = {
            checkDupEmail: true,
            theme: "Bootstrap",
            position: "top"
        ...
        return this.each(function () {
            ...
            var $orig = $(this);
            var $list = $('<div class="multiple_emails-ul" />'); // create html elements - list of email addresses as unordered list

            if ($(this).val() != '' && IsJsonString($(this).val())) {
                $.each(jQuery.parseJSON($(this).val()), function (index, val) {
                   // alert("inside js fn");

                    $list.append($('<div class="multiple_emails-email"><span class="email_name" data-email="' + val.toLowerCase() + '">' + val + '</span></div>')
                ...
                if (gShareType != 'BB') {
                    ...
                    for (var i = 0; i < arr.length; i++) {
                        ...
                        if (dupEmailCheck === true && $orig.val().indexOf(arr[i]) != -1) {
                            ...
                            $list.append($('<div class="multiple_emails-email"><span class="email_name" data-email="' + arr[i].toLowerCase() + '" data-key="' + $("#SelectedUserId").val() + '" >' + arr[i] + '</span></div>')
							  .prepend($(deleteIconHTML)
								   .click(function (e) { $(this).parent().remove(); refresh_emails(); e.preventDefault(); })
							  )
						);
...
//                alert("inside refresh_emails");
                var emails = new Array();
                var container = $orig.siblings('.multiple_emails-container');
                if (gShareType != 'BB') { //Email Panel Display Then this code
                    container.find('.multiple_emails-email span.email_name').each(function () { emails.push($(this).html()); }); //Only Email Addresses.
                }
                else {
                    container.find('.multiple_emails-email span.email_name').each(function () { emails.push($(this).attr("data-key")) }); //Only User Id's
                }
