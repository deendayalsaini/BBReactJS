...
function ConvertARADataUnit(sDataValue)
{
    var t= (parseFloat((sDataValue).replace(/,/g, '').replace('%', '').replace(/NA/g, null)) * gARA_Unit).toFixed(2).toString();      
    if(t== 'NaN')
    {
        return sDataValue;
    }
...
function ConvertARADataUnitChart(sDataValue)
{
    var t= (parseFloat((sDataValue).replace(/,/g, '').replace('%', '').replace(/NA/g, null)) * gARA_Unit).toFixed(2).toString();      
    if(t== 'NaN')
    {
...
$(document).ready(function () {
    ...
    function ARAResursive(pobj, pparentid) {
        try {
            for (var j = 0; j < pobj.length; j++) {
                ...
                $.each(ObjVal.DATA_VAL, function (index, slist) {
                    if(index >=gARA_StartIndex && index <= gARA_EndIndex)
                    {
                    MyTempArray.push(ConvertARADataUnitChart(slist));
                ...
                if (checkDataVal == MyTempArray.length) {
                    ...
                    var trC = $("<tr></tr>").addClass("treegrid-" + lgrdId).addClass("treegrid-parent-" + pparentid).appendTo($('#grvAnnualRatioAna'));
                    ...
                    if(index >=gARA_StartIndex && index <= gARA_EndIndex)
                    {
                        var td = $("<td></td>").html('<div   class=\"treegrid-Cell\" ' + ProfitStyle + '>' + ConvertARADataUnit(sYearData) + '</div>');
                        trC.append(td);
                    }
                    });
                    if (ObjVal.CHILD_DATA != null || ObjVal.CHILD_DATA.length > 0) {
                        ARAResursive(ObjVal.CHILD_DATA, lgrdId);
                    }
                }
            }
        } catch (msg) {
...
$('#tableContainerAnnualRatio').html()
$('#tableContainerAnnualRatio').html('<table id="grvAnnualRatioAna" class="tree QAtree table table-striped fixedHeader_collapsetree" style="width: 100% !important;" ></table>')
;

        $.each(gARA_DATA, function (index, ldata) {

            QValuationData = ldata.PANEL_DATA;

            if (QValuationData.LEVELDATA.length > 0) {

                var trHead = $("<thead class='gridviewtheadfixhd'></thead>").addClass("treegrid-" + index + "--0").appendTo($('#grvAnnualRatioAna'));

                var trH = $("<tr ></tr>").addClass("treegrid-" + index + "--0").appendTo($('#grvAnnualRatioAna'));
                trHead.append(trH);
            ...
            for (var i = 0; i < QValuationData.LEVELDATA.length; i++) {
                ...
                if (checkDataVal == MyTempArray.length) {
                    ...
                    var tr = $("<tr></tr>").addClass("treegrid-" + lTopGridId).appendTo($('#grvAnnualRatioAna'));
                    ...
                    $.each(lLevelData.DATA_VAL, function (index, sDataValue) {
                        if(index >=gARA_StartIndex && index <= gARA_EndIndex)
                       {
                        var td = $("<td></td>").html('<div  class=\"treegrid-Cell\"' + ProfitStyle + ' >' + ConvertARADataUnit(sDataValue) + '</div>');
                        tr.append(td);
                       }
                    });
                }
                ...
                if (lLevelData.CHILD_DATA != null) {
                    ...
                    for (var j = 0; j < lChildData.length; j++) {
                        ...
                        if (checkChildDataVal == MyTempArray.length) {
                            ...
                            var trC = $("<tr></tr>").addClass("treegrid-" + lgrdId).addClass("treegrid-parent-" + lTopGridId).appendTo($('#grvAnnualRatioAna'));
                            var tdC = $("<td ></td>").html("<div class=\"treegridHeaderColumn\" " + ProfitStyle + " title=\"" + ObjVal.FULLFIELD + "\"> " + ObjVal.FIELD + "</div>");
                            trC.append(tdC);
        ...
        /*ARAResursive function for bind child data*/


        $('#grvAnnualRatioAna').treegrid();
        ...
        var num;
        var $tds;
        $("#grvAnnualRatioAna").each(function (i, t) {
            $tds = $("td", t);
            $ths = $("th", t);
            ...
            .eq(1).css("min-width", "60px").end();
            //var tds = $("#grvAnnualRatioAna").children('thead').children('tr').children('th').length;
        ...
        //$("#grvAnnualRatioAna").fixedHeaderTable({ altClass: 'odd', footer: true, fixedColumns:0 });
